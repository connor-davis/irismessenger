 ...
 * the receivers publicKey and it will add a
 * signature for verification.
 */
export let encryptedPacket = ({ packet, publicKeys, signature }) => {
    return crypto.encrypt(publicKeys, JSON.stringify(packet), signature);
}

/**
 * This function will decrypt the packet using
 * the receivers privateKey and the signature
 ...
 */
export let verifyPacket = ({ packet, publicKeys }) => {
    let verified = false;

    publicKeys.forEach((key) => {
        verified = crypto.verify(key, packet.signature, packet.content);
    });
