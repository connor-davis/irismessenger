...
import {decryptedPacket, encryptedPacket, sign, verifyPacket} from './encryption';
...
export let recieve = ({reciever, dispatch}) => {
  _io.on(`_${reciever.id}NewMessage`, (packet) => {
    packet = decryptedPacket({packet: JSON.parse(packet), privateKey: reciever.keys.privateKey});
